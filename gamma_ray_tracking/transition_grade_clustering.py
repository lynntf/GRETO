...
from .physics import MEC2, RANGE_PROCESS, KN_vec, lin_att_abs, lin_att_compt
...
def format_three_aggregations(g: np.ndarray) -> np.ndarray:
    ...
    """
    out = np.stack((format_aggregation(g, dim=0),
                 format_aggregation(g, dim=1),
                 format_aggregation(g, dim=2)), axis=-2)
    return out
...
def get_grade_features(event: Event,
                       subset: List = None,
    ...
    E_ijk = event.tango_estimates[subset[:, np.newaxis, np.newaxis],
    ...
    one_minus_cosine_ijk = 1 - event.cos_act[subset[:, np.newaxis, np.newaxis],
                                             subset[np.newaxis, :, np.newaxis],
                                             subset[np.newaxis, np.newaxis, :]]
    ...
    ind = log_P_ij > 0
    P_angle[ind] = -np.log(KN_vec(E_imo=E_ijk[ind],
    ...
    for P in (P_en, P_en_continue, log_P_ij, log_P_jk, P_angle,
              E_ijk, E_ijk_mj, E_ijk_mj_mk, D_ij, D_jk,
              one_minus_cosine_ijk, two_int_one_minus_cos_theo,
              square_cosine_diff, e_i, e_j, e_k,):
    ...
    return np.stack((P_en, P_en_continue, log_P_ij, log_P_jk, P_angle,
                     E_ijk, E_ijk_mj, E_ijk_mj_mk, D_ij, D_jk,
                     one_minus_cosine_ijk, two_int_one_minus_cos_theo,
                     square_cosine_diff, e_i, e_j, e_k), axis = -1)

"""
We create this data with the following ideas in mind:
- We can train a machine learning model to distinguish between absorption interactions and scattering interactions:
    - We do this by selecting data from true absorptions and true scatters and training a classifier on it. It is challenging to select additional data outside of true absorptions and true scatters for this case because we cannot easily evaluate what some false series of interactions should be classified as.
- We can train a machine learning model to distinguish between true scattering events and false scattering events.
    - We have to decide whether false scattering events within a cluster should be included (for the sake of clustering, we do not really care; for the sake of ordering, we care).
    - This model should output a degree three transition tensor indicating if i->j->k is a proper scatter.
