...
from .fom_tools import get_all_features_cluster, individual_FOM_feature_names
from .event_tools import split_event_clusters
    ...
    **kwargs
    ...
    for cluster_id, cluster in true_clusters.items():
        ...
        if width is None:
            ...
            if len(cluster) == width + 1:  # special case where there is no savings for eliminating final element
                cluster_width = len(cluster)
        for pred_cluster in permutations(cluster, r = cluster_width):
            pred_cluster = list(pred_cluster)
            new_features = get_fom_pieces(event, pred_cluster, start_energy = energy_sum, **kwargs)
            ...
            if len(cluster) > 1:
                acceptable.append(pred_cluster[0] == cluster[0] and pred_cluster[1] == cluster[1])
    ...
    return (features, optimal_order, complete_energy, energy_sums, lengths,
            correct_solution_index, other_solution_index, cluster_ids, acceptable, true_cluster_ids)
...
def generate_semi_greedy_data_single_cluster(
    ...
    **kwargs
    ...
    return (features, optimal_order, acceptable, first_int_good,)
...
def build_data_single_rays(
    single_ray_events:List[Event],
    single_ray_clusters:List[Dict],
    width:int = 5
    ...
    for (i, event), cluster in zip(enumerate(single_ray_events), single_ray_clusters):
        (features, ordered, acceptable, first) = generate_semi_greedy_data_single_cluster(event, cluster, width = width)
...
def generate_all_data(
    ...
    **kwargs
    ...
    return X, y, e, lengths, correct_solution_index, index2, cid, acceptable, optimal_order
...
def make_data(
    ...
    **kwargs
    ...
    for (event_id, event), cluster in zip(enumerate(packed_smeared_events), packed_clusters):
        x, order, comp, e, l, i1, i2, cid, acc, t_cid = generate_semi_greedy_data(event,
                                                              cluster,
    ...
    return (
        features, ordered, complete, energy_sums, lengths, opt_index, other_index,
        cluster_ids, acceptable,
        event_ids, true_cluster_ids,
        )
...
def make_residuals(
    ...
    r_train = np.nan_to_num(X[other_index[train_label]]) - np.nan_to_num(X[opt_index[train_label]])
    ...
    I_val = I[val_label]
    r_val = np.nan_to_num(X[other_index[val_label]]) - np.nan_to_num(X[opt_index[val_label]])
...
def make_train_label(I:np.ndarray, val_ratio:float = 0.33, seed:int = 42) -> np.ndarray:
    """
    ...
    for event, clustering in zip(events, clusters):
        ...
        for cluster_id, cluster_energy_sum in energy_sum.items():
            ...
            if event.points[1].interaction_type is not None:
                pair_prod[cluster_id] = any([event.points[i].interaction_type > 2 for i in clustering[cluster_id]])
            else:
