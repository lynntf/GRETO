...
def cos_act(p1:np.ndarray[float], p2:np.ndarray[float], p3:np.ndarray[float]) -> float:
    """
    ...
    np.clip(cos_theta_act, -1., 1., out=cos_theta_act)
    return cos_theta_act

def theta_act(p1:np.ndarray[float], p2:np.ndarray[float], p3:np.ndarray[float]) -> float:
    """
...
def cosine_vec(points:np.ndarray[float],
               center:np.ndarray=np.array([0.,0.,0.])) -> np.ndarray[float]:
    """
...
def one_minus_cosine_ijk(points:np.ndarray[float]) -> np.ndarray[float]:
    ...
    o_m_cosine_ijk[np.logical_or(np.isnan(o_m_cosine_ijk), ~np.isfinite(o_m_cosine_ijk))] = 0
    return o_m_cosine_ijk

def err_cos_vec(points:np.ndarray[float],position_err:np.ndarray[float]=0.5) -> np.ndarray[float]:
    """
...
def partial_err_theta_vec_precalc(distances:np.ndarray[float],
                        cos_ijk:np.ndarray[float],
                        position_err:np.ndarray[float]=0.5) -> Tuple[np.ndarray[float]]:
    """
    ...
    if isinstance(sq_position_err, float):
        ...
        sigma_squared_a = (sq_position_err[:, np.newaxis, np.newaxis] + \
                           sq_position_err[np.newaxis, :, np.newaxis])/(rab**2)
        sigma_squared_b = 2*sq_position_err[np.newaxis, :, np.newaxis]*(cos_ijk)/(rab*rbc)
        sigma_squared_c = (sq_position_err[np.newaxis, :, np.newaxis] + \
                           sq_position_err[np.newaxis, np.newaxis, :])/(rbc**2)
    return (sigma_squared_a, sigma_squared_b, sigma_squared_c)

def err_cos_vec_precalc(distances:np.ndarray[float],
                        cos_ijk:np.ndarray[float],
                        position_err:np.ndarray[float]=0.5) -> np.ndarray[float]:
    """
    # Error propagation in the computation of the cos(theta).
    ...
    ## Returns:
        - Propagated error
    """
    sa, sb, sc = partial_err_theta_vec_precalc(distances=distances,
                                               cos_ijk=cos_ijk,
                                               position_err= position_err)
    return np.sqrt((1 - (cos_ijk)**2)*(sa + sb + sc))

def err_theta_vec_precalc(distances:np.ndarray[float],
                        cos_ijk:np.ndarray[float],
                        position_err:np.ndarray[float]=0.5) -> np.ndarray[float]:
    """
...
def ge_distance(points:np.ndarray[float],
                inner_radius:float=None,
                d12_euc:np.ndarray = None,
                detector:DetectorConfig=default_config) -> np.ndarray[float]:
    """
    ...
    lambda_1 = d1 - d
    lambda_2 = d2 - d
    ind = np.logical_and.reduce((indicator, 0 <= lambda_1, 0 <= lambda_2,
                                 lambda_1 <= d12_euc, lambda_2 <= d12_euc))
    np.add(d12_euc, -2*d, where=ind, out=d12_euc)
    ind = np.logical_and.reduce((indicator, 0 <= lambda_1, lambda_2 < 0, lambda_1 <= d12_euc))
    np.add(d12_euc, -d - d2, where=ind, out=d12_euc)
    ind = np.logical_and.reduce((indicator, lambda_1 < 0, 0 <= lambda_2, lambda_2 <= d12_euc))
    np.add(d12_euc, -d - d1, where=ind, out=d12_euc)
    ind = np.logical_and.reduce((indicator, lambda_1 < 0, lambda_2 < 0))
    d12_euc[ind] = 0.0
    return d12_euc

def germanium_extension(p1:np.ndarray[float],
                        p2:np.ndarray[float],
                        inner_radius: float = None,
                        detector:DetectorConfig=default_config) -> np.ndarray[float]:
    """
    ...
    if inner_radius is None:
        inner_radius = detector.inner_radius
    half_width = np.sqrt(inner_radius**2 - np.linalg.norm(p1 - direction*np.dot(p1, direction))**2)
...
def cartesian_to_spherical(xyz:np.ndarray) -> np.ndarray:
    ...
    rpt[:,1] = -np.arctan2(xyz[:,2], np.sqrt(xy)) # for elevation angle defined from XY-plane up
...
class cone:
    ...
    def __init__(self, apex:np.ndarray, direction:np.ndarray, opening_angle:float,
                 sphere_radius:float):
        """
        ...
        self.c = np.dot(self.apex, self.apex) - sphere_radius ** 2
        self.mag_a = np.sqrt(np.dot(self.apex, self.apex))
        self.beta = np.arccos(np.dot(self.apex, self.direction)/self.mag_a) # angle between apex and axis
        self.c1 = np.sin(self.beta) * np.sin(self.opening_angle)
        self.c2 = np.cos(self.beta) * np.cos(self.opening_angle)

    def ray_lengths(self, theta):
        """
        Trig method for computing ray lengths. Theta here does not necessarily
        match ray construction
        """
        cos_phi = self.c1 * np.cos(theta) + self.c2
        t = - self.mag_a *cos_phi + np.sqrt(self.mag_a**2 * (cos_phi**2 - 1) + self.sphere_radius**2)
        return t
    ...
    def _get_first_ray(self):
        ...
        first_ray = Rotation.from_rotvec(self.opening_angle * first_axis).apply(self.direction)
...
def cone_ray_lengths(apex: np.ndarray, direction: np.ndarray, opening_angle: float,
                     num_rays: int, radius: float) -> np.ndarray:
    """
    ...
    ray_directions = Rotation.from_rotvec(thetas[:, np.newaxis] * direction[np.newaxis, :]).apply(first_ray)
    # ray_directions /= np.linalg.norm(ray_directions, axis=1)[:, np.newaxis]
...
def cone_ray_length(apex: np.ndarray, direction: np.ndarray, opening_angle: float,
                     theta: float, radius: float) -> np.ndarray:
    """
...
def cone_ray_length_2(apex: np.ndarray, direction: np.ndarray, opening_angle: float,
                     theta: float, sphere_radius: float) -> np.ndarray:
    """
    Initializes a cone with the given apex, direction, and opening angle.
...
def crystal_depth(global_coords: np.ndarray,
    ...
    c_depth = r_from_target * z_crystal / np.sqrt(r_from_target**2 - r_from_crystal_axis**2)
    return c_depth
