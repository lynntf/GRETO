...
from ..cluster_utils import get_all_clusters, in_peak
from ..event_class import Event


def create_hierarchical_cluster_problem(event:Event, observed_energies:List[float],
        epsilon:float=0.005):
    """
    ...
    peak_clusters = [cluster for (i, cluster) in all_clusters.items()
    ...
    expr = sum((1 + eps*len(peak_clusters[i-1]))*model.u[i]
               for i in model.cluster_idxs) # pylint: disable=E1133
    model.obj = pyenv.Objective(expr=expr, sense=pyenv.maximize)

    return model

def hierarchical_cluster(event, S, observed_energies, alpha=0.4,
                         epsilon=0.005, solver='cplex'):
    """
    ...
    model = create_hierarchical_cluster_problem(event, observed_energies,
                                                epsilon)
    ...
    if model is not None:
        ...
        clusters = sorted([sorted(model.peak_clusters[i-1]) for i in model.u
                                   if pyenv.value(model.u[i]) > 0.5])
    else:
        # If it is None, then, the clustering found no peak clusters
        clusters = []

    remaining_points = [i for i in range(1, len(event.points)) 
                        if all(i not in cluster for cluster in clusters)]
    remainder_event = event.subset(remaining_points)
    remainder_clusters = remainder_event.cluster_linkage(
            alpha, S-len(clusters))
