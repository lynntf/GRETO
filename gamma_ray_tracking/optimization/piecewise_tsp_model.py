...
def create_piecewise_tsp_tracking_problem(
    ...
    def energy_drop_rule(model, j):
        return model.E[j] == model.E[j - 1] - sum(
            model.e[i] * model.z[i, j] for i in model.I
        )
    ...
    def w_rule_4(model, i, j, k, m):
        return (
            model.w[i, j, k, m]
            >= model.z[i, m - 1] + model.z[j, m] + model.z[k, m + 1] - 2
        )

    model.w_4 = pyenv.Constraint(model.w_index, rule=w_rule_4)

    def act_angle_rule(model, l):
        return model.c_act[l] == sum(
            model.c[i, j, k] * model.w[i, j, k, l] for (i, j, k) in model.angles
        )

    model.act_angle = pyenv.Constraint(model.I_minus, rule=act_angle_rule)

    def fom_rule(model):
        return (1 / (model.N - 1)) * sum(
            (model.c_theo[i] - model.c_act[i]) ** 2 for i in model.I_minus
        )
...
def warm_start(instance: pyenv.Model, permutation):
    ...
    for i, j, k in instance.angles:
        for m in instance.I_minus:
            if (
                instance.z[i, m - 1].value > 1 - 1e6
                and instance.z[j, m].value > 1e-6
                and instance.z[k, m + 1].value > 1e-6
            ):
    ...
    for i in instance.I_minus:
        m = i
        instance.c_act[m] = sum(
            instance.c[i, j, k] * instance.w[i, j, k, m].value
            for (i, j, k) in instance.angles
        )
