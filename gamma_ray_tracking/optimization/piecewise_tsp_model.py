...
Piecewise traveling salesperson problem
...
def create_piecewise_tsp_tracking_problem(event:Event,
                                          pw_breaks, pw_repn) -> pyenv.ConcreteModel:
    """Create the pyomo model of the problem"""
    ...
    def act_angle_rule(model, l):
        return model.c_act[l] == sum(model.c[i,j,k] * model.w[i,j,k,l]
                                     for (i, j, k) in model.angles)
    model.act_angle = pyenv.Constraint(model.I_minus, rule=act_angle_rule)

    def fom_rule(model):
        return (1/(model.N-1))* sum((model.c_theo[i] - model.c_act[i])**2
                                    for i in model.I_minus)
    ...
    for i in range(model.N):
        attr_name = f'p{i}'
        setattr(model, attr_name, pyenv.Piecewise(model.E_inv[i], model.E[i], pw_pts=xs,
                                                  pw_constr_type='EQ', f_rule=ys, pw_repn=pw_repn))

    return model

def warm_start(instance:pyenv.Model, permutation):
    ...
    for i, p_idx in enumerate(permutation):
        ...
        instance.z[p_idx,i+1] = 1
        for j in instance.I:
            if j != i+1:
                instance.z[p_idx,j] = 0

    # Initialize E
    instance.E[0] = instance.E0.value
    for i, p_idx in enumerate(permutation):
        instance.E[i+1] = round(instance.E[i].value - instance.e[p_idx], 3)

    # Initialize w
    for (i, j, k) in instance.angles:
        for m in instance.I_minus:
            if instance.z[i,m-1].value > 1-1e6 and \
                instance.z[j,m].value > 1e-6 and instance.z[k,m+1].value > 1e-6:
    ...
    for i in instance.I_minus:
        m = i
        instance.c_act[m] = (sum(instance.c[i,j,k]*instance.w[i,j,k,m].value
                                 for (i,j,k) in instance.angles))
        instance.c_theo[i] = 1 - instance.m0c2.value * \
            (1/instance.E[i].value - 1/instance.E[i-1].value)
