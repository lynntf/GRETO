...
from .. import cluster_utils, default_config
from ..detector_config_class import DetectorConfig
from ..event_class import Event
from ..event_tools import flatten_event
...
def plot_clusters_2d(event, clusters, ax=None, set_global=True,
                     unordered=False, with_labels=False):
...
def plot_clusters_cross_sections(event:Event, clusters:Dict,
...
def plot_flat_event(event:Event, clusters:dict,
    ...
    if include_origin:
        for cluster_id, cluster in clusters.items():
            plt.plot(event.point_matrix[[0] + list(cluster), 0],
                    event.point_matrix[[0] + list(cluster), 1],
                    marker='.', **plot_kwargs)
            plt.scatter(event.point_matrix[cluster[0],0],
                        event.point_matrix[cluster[0],1],
                        marker='o', color='black', s=30)
            if with_labels:
                ax.text(event.point_matrix[cluster[0],0],
                               event.point_matrix[cluster[0],1],
                               str(cluster_id))
    else:
        for cluster_id, cluster in clusters.items():
            plt.plot(event.point_matrix[list(cluster), 0],
...
def plot_flat_event_px(event:Event, clusters:dict,
                       include_origin:bool=True,
                       include_outer_radius:bool=True,
                       detector:DetectorConfig=default_config):
    """
    ...
    for cluster_id, cluster in clusters.items():
        if include_origin:
            dashed_df = pd.DataFrame(event.point_matrix[[0] + [cluster[0]]], columns=['x','y','z'])
        ...
        scatter_trace.marker.symbol = [highlight_shape if i == 0 else 'circle'
...
#%% 3D plots
def plot_clusters_3d(event:Event, clusters:Dict, include_origin:bool=False,
                     detector:DetectorConfig=default_config):
    ...
    for s in clusters.keys():
        if len(clusters[s]) > 0:
            ...
            if include_origin:
                ax.plot([0,xs[0]], [0, ys[0]], [0,zs[0]],
                        color=colors[s % len(colors)])
            if len(clusters[s]) == 1:
                ax.scatter(xs, ys, zs,
                           color=colors[s % len(colors)], marker='.')
...
def plot_event_3d(event:Event, clusters:dict, include_origin:bool=True,
    ...
    for cluster_id, cluster in clusters.items():
        ...
        df['hover_info'] = list(zip(df['id'],
                                ...
                                df['angle'],
                                df['tot_energy'],
                                df['type']))  # Store all hover information in a tuple
...
def plot_points_3d(event:Event, # point_matrix:np.ndarray = None,
                  include_inner_radius:bool=True,
                  include_outer_radius:bool=False,
                  detector:DetectorConfig=default_config):
    """Method for plotting flat events (the third dimension has been removed)"""
    ...
    df['hover_info'] = list(zip(df['id'],
                            df['energy']))  # Store all hover information in a tuple
...
def plot_bad_clustering_sample(list_of_events:List[Event],
                               bad_idxs:List[int],
...
def plot_right_wrong_histogram(events, tracks, true_tracks):
    ...
    mismatch_energies = cluster_utils.compute_mismatch_counts(events, tracks, true_tracks)
    energies = {'Right': mismatch_energies[0],
                'Wrong': sum([mismatch_energies[i]
                 for i in mismatch_energies if i > 0], start=[])}
    plt.figure()
    max_energy = max(max(v) for v in energies.values())
    sns.histplot(energies,
                 bins=np.arange(0, max_energy + 0.004, 0.004), element='step', alpha=0.3)
    plt.yscale('log')


def plot_energy_movement_histogram(events, tracks, true_tracks, observed_energies, **kwargs):
    ...
    hist_kwargs = {'bins': np.arange(0,max_energy + 0.004, 0.004), 'element': 'step', 'alpha': 0.3}
...
def pt_eff(
    ...
    if peak_data_indicator is None:
        peak_data_indicator = detect_peaks(energy_data, peak_energies, peak_energy_tolerance)
    non_peak_indicator = np.logical_not(peak_data_indicator)
    quantity_peak, t = resampled_cumulant(
        np.nan_to_num(foms[peak_data_indicator]),
        t = fom_range,
        normalize=False
        )
    quantity_non_peak, t = resampled_cumulant(
        np.nan_to_num(foms[non_peak_indicator]),
        t = t,
        normalize=False
        )
...
def detect_peaks(energies:np.ndarray, peak_energies:list[float], tol:float=1e-2) -> np.ndarray:
