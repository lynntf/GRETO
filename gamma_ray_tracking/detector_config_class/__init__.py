...
def read_greta_crmat(filename):
    ...
    with importlib.resources.open_binary(__name__, filename) as file:
        crmat = np.array(struct.unpack('f'*16*124,
                                       file.read(struct.calcsize('f'*16*124)))).reshape((124, 4, 4))
    return crmat
...
def read_agata_crmat(filename):
    ...
    with importlib.resources.open_text(__name__, filename) as file:
        ...
        for _ in range(180*4):
            ...
            if len(line) == 5:
                crmat.append([float(value) for value in line[2:] + [0.]])
            if len(line) == 4:
                crmat[-1].extend([float(value) for value in line[1:] + [0.]])
...
class DetectorConfig:
    ...
    """
    def __init__(self, detector:str='agata', config_file='detector_config.json',
                 position_error_model:str='siciliano'):
        with importlib.resources.open_text(__name__,config_file,
                                           encoding="utf-8") as f:
            config = json.load(f)
        ...
        self.position_error_w0_Soederstroem = config['POSITION_ERROR_W0_cm_Soederstroem']
        self.position_error_w1_Soederstroem = config['POSITION_ERROR_W1_cm_Soederstroem']
        ...
        self.origin = Interaction([config['default_origin_x_cm'],
                                   config['default_origin_y_cm'],
                                   config['default_origin_z_cm']],
                                  0.,
                                  crystal_no=0,
                                  crystal_x=[config['default_origin_x_cm'],
                                             config['default_origin_y_cm'],
                                             config['default_origin_z_cm']],
                                  interaction_type=0)
    ...
    def set_radius(self, inner:float=0., outer:float=0.):
        """
    ...
    def set_detector(self, detector:str):
        ...
        if detector.lower().startswith('agata'):
            self.set_radius(inner=self.agata_inner_radius, outer=self.agata_outer_radius)
            self.crmat = self.agata_crmat
            self.detector = "AGATA"
        elif detector.lower().startswith('gret'):
            self.set_radius(inner=self.greta_inner_radius, outer=self.greta_outer_radius)
    ...
    def position_error(self, energy: np.ndarray):
        ...
        out[energy > 0] = out[energy > 0] + self.position_error_w1*np.sqrt(0.100/energy[energy > 0])
        return out
