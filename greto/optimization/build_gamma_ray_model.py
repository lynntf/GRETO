...
def evaluate_piecewise(x: float, xs: Iterable[float], ys: Iterable[float]) -> float:
    ...
    y = ys[index - 1] + (ys[index] - ys[index - 1]) / (xs[index] - xs[index - 1]) * (
        x - xs[index - 1]
    )
    return y
...
def build_model(
    ...
    alpha: float = 0.2,
    symmetry: bool = False,
    piecewise_error: float = 0.1,
    x_0: float = 0.01,
    x_max: float = 10,
    singleton_penalty: float = 0,
    energy_big_M: bool = False,
    pw_repn: str = "SOS2",
    add_escape_event: bool = False,
) -> pyenv.ConcreteModel:
    """
    ...
    model.c_theo = pyenv.Var(
        model.J_minus, model.clusters, within=pyenv.Reals, bounds=(-1, 1)
    )
    model.c_act = pyenv.Var(
        model.J_minus, model.clusters, within=pyenv.Reals, bounds=(-1, 1)
    )

    if FOM_method == "avg":
        model.r_theo = pyenv.Var(model.J_minus, model.J, model.clusters, bounds=(-1, 1))
        model.r_act = pyenv.Var(model.J_minus, model.J, model.clusters, bounds=(-1, 1))
    ...
    model.z = pyenv.Var(model.I0, model.J0, model.clusters, within=pyenv.Binary)

    model.w_index = model.angles * model.J_minus * model.clusters
    model.w = pyenv.Var(model.w_index, bounds=(0, 1))
    model.E = pyenv.Var(
        model.J0, model.clusters, within=pyenv.NonNegativeReals, bounds=(0, 10)
    )
    model.E_inv = pyenv.Var(model.J0, model.clusters, within=pyenv.NonNegativeReals)
    ...
    model.each_point_in_path = pyenv.Constraint(
        model.I0, model.clusters, rule=each_point_in_path_rule
    )
    ...
    model.one_j_in_path = pyenv.Constraint(
        model.J0, model.clusters, rule=one_j_in_path_rule
    )
    ...
    if energy_big_M:
        ...
        model.energy_drop_1 = pyenv.Constraint(
            model.J_minus, model.clusters, rule=energy_drop_rule_1
        )
        ...
        model.energy_drop_2 = pyenv.Constraint(
            model.J_minus, model.clusters, rule=energy_drop_rule_2
        )
        ...
        model.theo_angle = pyenv.Constraint(
            model.J_minus, model.clusters, rule=theo_angle_rule
        )

    else:
        # Energy drops at each stop by energy in point j
        def energy_drop_rule(model, j, s):
            rhs = model.E[j - 1, s] - sum(
                model.e[i] * model.z[i, j, s] for i in model.I
            )
            return model.E[j, s] == rhs

        model.energy_drop = pyenv.Constraint(
            model.J, model.clusters, rule=energy_drop_rule
        )
        ...
        def theo_angle_rule_1(model, j, s):
            rhs = (
                1
                - model.m0c2 * (model.E_inv[j, s] - model.E_inv[j - 1, s])
                + 100 * (1 - model.v[j + 1, s])
            )
            return model.c_theo[j, s] <= rhs

        model.theo_angle_1 = pyenv.Constraint(
            model.J_minus, model.clusters, rule=theo_angle_rule_1
        )
        ...
        model.theo_angle_2 = pyenv.Constraint(
            model.J_minus, model.clusters, rule=theo_angle_rule_2
        )
    ...
    model.origin_start = pyenv.Constraint(model.clusters, rule=origin_start_rule)

    # Each cluster includes the origin
    def origin_start_rule_2(model, s):
        return model.u[0, s] == 1

    model.origin_start_2 = pyenv.Constraint(model.clusters, rule=origin_start_rule_2)
    ...
    def act_angle_rule_1(model, j, s):
        rhs = sum(
            model.c[angle] * model.w[angle, j, s] for angle in model.angles
        ) + 2 * (1 - model.v[j + 1, s])
        return model.c_act[j, s] <= rhs

    model.act_angle_1 = pyenv.Constraint(
        model.J_minus, model.clusters, rule=act_angle_rule_1
    )
    ...
    model.act_angle_2 = pyenv.Constraint(
        model.J_minus, model.clusters, rule=act_angle_rule_2
    )
    ...
    if FOM_method == "avg":
        ...
        model.mccormick_1_act = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_1_act
        )

        def mccormick_rule_2_act(model, i, j, s):
            return model.y[j, s] - model.r_act[i, j, s] >= 0

        model.mccormick_2_act = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_2_act
        )

        def mccormick_rule_3_act(model, i, j, s):
            return model.c_act[i, s] - model.r_act[i, j, s] + 1 - model.y[j, s] >= 0

        model.mccormick_3_act = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_3_act
        )

        def mccormick_rule_4_act(model, i, j, s):
            return 1 - model.c_act[i, s] - model.y[j, s] + model.r_act[i, j, s] >= 0

        model.mccormick_4_act = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_4_act
        )

        def mccormick_rule_1_theo(model, i, j, s):
            return model.r_theo[i, j, s] + model.y[j, s] >= 0

        model.mccormick_1_theo = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_1_theo
        )

        def mccormick_rule_2_theo(model, i, j, s):
            return model.y[j, s] - model.r_theo[i, j, s] >= 0

        model.mccormick_2_theo = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_2_theo
        )

        def mccormick_rule_3_theo(model, i, j, s):
            return model.c_theo[i, s] - model.r_theo[i, j, s] + 1 - model.y[j, s] >= 0

        model.mccormick_3_theo = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_3_theo
        )

        def mccormick_rule_4_theo(model, i, j, s):
            return 1 - model.c_theo[i, s] - model.y[j, s] + model.r_theo[i, j, s] >= 0

        model.mccormick_4_theo = pyenv.Constraint(
            model.J_minus, model.J, model.clusters, rule=mccormick_rule_4_theo
        )
        ...
        model.cluster_count = pyenv.Constraint(model.clusters, rule=cluster_count_rule)
    ...
    if error == "l1":
        if FOM_method == "avg":
            ...
            def fom_rule(model):
                return sum(
                    (
                        sum(
                            1 / (j - 1) * model.error[i, j, s]
                            for j in pyenv.RangeSet(2, model.M)
                        )
                    )  # pylint: disable=E1133
                    for i in model.J_minus
                    for s in model.clusters
                ) + singleton_penalty * sum(model.y[1, s] for s in model.clusters)

            model.l1_FOM_upper = pyenv.Constraint(
                model.J_minus, model.J, model.clusters, rule=l1_FOM_upper_rule
            )
            model.l1_FOM_lower = pyenv.Constraint(
                model.J_minus, model.J, model.clusters, rule=l1_FOM_lower_rule
            )
        elif FOM_method == "sum":
            ...
            def fom_rule(model):
                return sum(
                    model.error[j, s] for j in model.J_minus for s in model.clusters
                ) + singleton_penalty * sum(model.y[1, s] for s in model.clusters)
        ...
        if FOM_method == "sum":
            ...
            def fom_rule(model):
                term1 = sum(
                    (model.c_theo[j, s] - model.c_act[j, s]) ** 2
                    for j in model.J_minus
                    for s in model.clusters
                )
                term2 = singleton_penalty * sum(model.y[1, s] for s in model.clusters)
                return term1 + term2
                ...
                return sum(
                    (
                        sum(
                            1
                            / pyenv.sqrt(j - 1)
                            * (model.r_act[i, j, s] - model.r_theo[i, j, s])
                            for j in pyenv.RangeSet(2, model.M)
                        )
                        ** 2
                    )  # pylint: disable=E1133
...
def warm_start(
    ...
    for s in instance.clusters:
        ...
        for j in instance.J0:
            if j == 0:
                instance.E[j, s] = sum(instance.e[p_idx] for p_idx in permutations[s])
            elif j < len(permutations[s]) + 1:
                p_idx = permutations[s][j - 1]
                instance.E[j, s] = max(
                    instance.E[j - 1, s].value - instance.e[p_idx], 0
                )
            else:
        ...
        # Initialize E_inv
        for j in instance.J0:
            instance.E_inv[j, s] = evaluate_piecewise(instance.E[j, s].value, xs, ys)

        # Initialize w
        for i, j, k in instance.angles:
            for m in instance.J_minus:
                if (
                    instance.z[i, m - 1, s].value > 1 - 1e-6
        ...
        for j in instance.J_minus:
            if j < len(permutations[s]):
                instance.c_act[j, s] = sum(
                    instance.c[angle] * instance.w[angle, j, s].value
        ...
        if FOM_method == "avg":
            for i in instance.J_minus:
                for j in instance.J:
                    instance.r_act[i, j, s] = (
                        instance.c_act[i, s].value * instance.y[j, s].value
                    )
                    instance.r_theo[i, j, s] = (
                        instance.c_theo[i, s].value * instance.y[j, s].value
                    )
            ...
            if error == "l1":
                for i in instance.J_minus:
                    for j in instance.J:
                        instance.error[i, j, s] = abs(
                            instance.r_theo[i, j, s].value
            ...
            if error == "l1":
                for j in instance.J_minus:
                    instance.error[j, s] = abs(
                        instance.c_theo[j, s].value - instance.c_act[j, s].value
                    )
...
def angular_distance(instance: pyenv.Model, i1, i2):
    ...
    dist = abs(
        math.acos(
            math.sin(instance.theta[i1])
            * math.sin(instance.theta[i2])
...
def add_distance_rule(instance: pyenv.Model):
    ...
    for i1, i2 in combinations(instance.I, 2):
        ...
        if dist > instance.alpha:
            for s in instance.clusters:
                instance.distance_constrs.add(
                    instance.u[i1, s] + instance.u[i2, s] <= 1
                )
