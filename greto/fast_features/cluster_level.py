...
def cluster_atoms(
    ...
    n = compute_value("n", [], lambda: len(cluster))
    index += 1
    ...
    # barycenter = compute_value(
    #     "barycenter",
    #     ["point_matrix", "energy_matrix"],
    #     lambda: geo.barycenter(event_calc.point_matrix[transition_1D]),
    # )
    # index += 1

    centroid_r = compute_value("centroid_r", ["centroid"], lambda: njit_norm(centroid))
    index += 1

    average_r = compute_value(
        "average_r", ["radii"], lambda: njit_mean(event_calc.radii[transition_1D[1:]])
    )
    index += 1
    ...
    length = compute_value("length", ["length_width"], lambda: length_width[-1])
    index += 1
