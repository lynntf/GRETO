...
def split_event_clusters(
    event: Event, clustering: Dict
...
def subset(
    event: Event, indices: Iterable, event_id: Any = None, reset_indices: bool = False
) -> Event:
    """
...
def flatten_cluster(
    ...
    for angle, dist in zip(angles, ge_distances[1:]):
        if randomize_heading:
            ...
            if np.dot(
                np.array([np.cos(theta + angle), np.sin(theta + angle), 0]), point_x[0]
            ) < np.dot(
                np.array([np.cos(theta - angle), np.sin(theta - angle), 0]), point_x[0]
            ):
                theta -= angle
...
def augment_remove_head(event: Event, clusters: Dict, n: int = 1):
    ...
    for cluster_index in clusters:
        if n < len(clusters[cluster_index]):
            pn = event.points[
                clusters[cluster_index][n - 1]
            ].x  # want to rotate about this point
            pnp1 = event.points[clusters[cluster_index][n]].x
            theta = event.theta_act_perm(
                [clusters[cluster_index][n - 1], clusters[cluster_index][n]]
            )
            ...
            dist = event.ge_distance[
                clusters[cluster_index][n - 1], clusters[cluster_index][n]
            ]

            def apply_rotation(vec, center, rotation, dist):
                return rotation.apply(vec - center) + (
    ...
    for cluster in clusters.values():
        for i, index in enumerate(cluster):
            ...
            for j in cluster[i + 1 :]:
                p = event.points[j]
                event.points[j] = p.update_position(
                    apply_rotation(event.points[j].x, pn, rot)
                )
